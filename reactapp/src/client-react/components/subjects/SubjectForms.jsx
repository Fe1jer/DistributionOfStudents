...
function SubjectDeleteForm({ onLoadData }) {
    ...
    const onSubmit = (e) => {
        if (id != "0") {
            ...
            xhr.open("delete", SubjectsApi.getDeleteUrl(id), true);
...
function SubjectUpdateForm({ onLoadData }) {
    ...
    const onSubmit = (e) => {
        ...
        if (id == "0") {
            ...
            xhr.open("put", SubjectsApi.getPutUrl(id), true);
        }
        xhr.setRequestHeader("Content-Type", "application/json")
        xhr.onload = function () {
            if (xhr.status === 200) {
